...
/// crap null stream from internets (https://stackoverflow.com/questions/8243743/is-there-a-null-stdostream-implementation-in-c-or-libraries)
...
void foo(UTF8 val) { std::cout << "foo val: " << val << std::endl; }
...
bool test_time_day_month_format(std::ostream &outstream) {
    ...
    Time t(date);
    outstream << "\t" << "timestamp format:      " << format << std::endl;
    outstream << "\t" << "timestamp from format: " << t.str(format) << std::endl;
    outstream << "\t" << "strtof version:        " << data << std::endl;
    bool retval = true;
...
bool test_time_date_format(std::ostream &outstream,
        std::string locale_format,
        std::string format,
        time_t time)
{
    bool retval = true;
    ...
    if(!(mytime.str(locale_format) == fromtime(time, format))) {
        std::cerr << "Format24HourMin errors: "
            << "["<<fromtime(time, format).length()<<"] "
            << fromtime(time, format)
            << " :vs: "
            << "["<<mytime.str(locale_format).length()<<"] "
            << mytime.str(locale_format)
            << std::endl;
        retval = false;
    }
...
bool test_time_format24hour_min(std::ostream &outstream) {
    ...
    outstream << "\t" << "systime:            " << SysTime() << std::endl;
...
bool test_time_from_before(std::ostream &outstream,
        time_t before,
        std::string format_expected)
{
    auto retval = true;
...
bool test_time_from_today_200s(std::ostream &outstream) {
    ...
    return test_time_from_before(outstream,
            timenow - 200,
            Locale::format(Locale::Format12HourMin));
}
...
bool test_time_from_today_49h(std::ostream &outstream) {
    ...
    return test_time_from_before(outstream,
            timenow - 3600*49,
            Locale::format(Locale::FormatLocaleDateFull));
}
