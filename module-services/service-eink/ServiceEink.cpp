...
namespace service::eink
{
    namespace
    {
        ...
        std::string toString(EinkModeMessage::Mode mode)
        {
    ...
    {
        const auto darkModeStr       = settings->getValue(settings::Display::darkMode);
        const auto isDarkModeEnabled = utils::toNumeric(darkModeStr);
        const auto mode = isDarkModeEnabled == 0 ? EinkModeMessage::Mode::Normal : EinkModeMessage::Mode::Invert;
        setDisplayMode(mode);
    }
    ...
    sys::MessagePointer ServiceEink::handleEinkModeChangedMessage(sys::Message *message)
    {
        ...
        setDisplayMode(mode);
        settings->setValue(settings::Display::darkMode, toString(mode));
    ...
    void ServiceEink::setDisplayMode(EinkModeMessage::Mode mode)
    {
        if (mode == EinkModeMessage::Mode::Normal) {
            ...
            internal::StaticData::get().disableDarkMode();
        }
        else {
            display.setMode(EinkDisplayColorMode_e::EinkDisplayColorModeInverted);
            internal::StaticData::get().enableDarkMode();
        }
    }
