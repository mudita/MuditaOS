...
// add messages to handle pre wake up
...
namespace alarms
{
    ...
    auto AlarmMessageHandler::handlePreWakeUpStatus(GetPreWakeUpRequestMessage *request)
        -> std::shared_ptr<GetPreWakeUpResponseMessage>
    {
        return handleWithCallback<GetPreWakeUpRequestMessage, GetPreWakeUpResponseMessage, bool>(
            request, [&](GetPreWakeUpRequestMessage *request, IAlarmOperations::OnPreWakeUpStatus callback) {
                alarmOperations->getPreWakeUpStatus(callback);
            });
    }
