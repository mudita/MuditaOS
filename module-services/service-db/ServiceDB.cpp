...
sys::Message_t ServiceDB::DataReceivedHandler(sys::DataMessage *msgl, sys::ResponseMessage *resp)
{
    ...
    case MessageType::DBSMSGetSMSLimitOffsetByThreadID: {
        ...
        timestamp = cpp_freertos::Ticks::GetTicks();
#endif
        auto ret = smsRecordInterface->GetLimitOffsetByField(msg->offset, msg->limit, SMSRecordField::ThreadID,
                                                             std::to_string(msg->id).c_str());
#if SHOW_DB_ACCESS_PERF == 1
    ...
    case MessageType::DBContactGetByName: {
        ...
        responseMsg = std::make_shared<DBContactResponseMessage>(
            std::move(ret), true, msg->limit, msg->offset, msg->favourite, ret->size(),
...
#endif
        // std::unique_ptr<std::vector<ContactRecord>> rec,uint32_t retCode=0,uint32_t  count=0,uint32_t respTo=0, bool
        // favourite = false
    ...
    case MessageType::DBContactGetLimitOffset: {
        ...
        if (msg->favourite)
            ret = std::move(contactRecordInterface->GetLimitOffsetByField(msg->offset, msg->limit,
                                                                          ContactRecordField::Favourite, "1"));
        else
            ret = std::move(contactRecordInterface->GetLimitOffset(msg->offset, msg->limit));
#if SHOW_DB_ACCESS_PERF == 1
        LOG_DEBUG("DBContactGetLimitOffset offset: %d, limit: %d size: %d, time: %lu favourites: %s", msg->offset,
                  msg->limit, ret->size(), cpp_freertos::Ticks::GetTicks() - timestamp,
                  (msg->favourite == true ? "TRUE" : "FALSE"));
#endif
