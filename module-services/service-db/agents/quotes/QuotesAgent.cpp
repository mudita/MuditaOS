...
namespace Quotes
{
    ...
    auto QuotesAgent::handleAddQuote(std::shared_ptr<db::Query> query) -> std::unique_ptr<db::QueryResult>
    {
        ...
        auto queryResult =
            database->execute(Queries::addQuote, req->langId, req->quote.c_str(), req->author.c_str(), req->enabled);
        auto quoteId  = database->getLastInsertRowId();
        auto response = std::make_unique<Messages::AddQuoteResponse>(queryResult, quoteId);
        response->setRequestQuery(query);
        return response;
    }
    ...
    auto QuotesAgent::handleDeleteQuote(std::shared_ptr<db::Query> query) -> std::unique_ptr<db::QueryResult>
    {
        ...
        auto queryResult = database->execute(Queries::deleteQuote, req->quoteId);
