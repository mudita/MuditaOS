...
TEST_CASE("Settings Messages")
{
    ...
    SECTION("Send register messages")
    {
        ...
        settings.bus.sendUnicast(std::make_shared<settings::Messages::RegisterOnVariableChange>(
                                     settings::EntryPath({"mode", "service", "profile", "variable"})),
                                 "db-worker");

        settings.bus.sendUnicast(std::make_shared<settings::Messages::UnregisterOnVariableChange>(
                                     settings::EntryPath({"mode", "service", "profile", "variable"})),
                                 "db-worker");
    }
