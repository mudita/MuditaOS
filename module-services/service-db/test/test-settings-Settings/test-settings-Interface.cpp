...
#include <memory>
#include <module-sys/Service/ServiceInterface.hpp>
...
class Interface_imp : public service::Interface
{
  public:
    using Interface::getApp;
    Interface_imp() = default;
    explicit Interface_imp(std::weak_ptr<sys::Service> s) : service::Interface(std::move(s))
    {}
};
...
TEST_CASE("Interface - not initialized")
{
    SECTION("base api")
    {
        auto interface = service::Interface();
    ...
    SECTION("inherited api")
    {
        ...
        REQUIRE_THROWS_AS(interface.getApp(), std::runtime_error);
    }
}
...
TEST_CASE("Interface - initialized")
{
    ...
    REQUIRE(interface.isValid());
    REQUIRE_NOTHROW(interface.getApp());
}
...
TEST_CASE("Interface - copied")
{
    ...
    REQUIRE_NOTHROW(interface2.getApp());
    REQUIRE(interface2.getApp() == interface.getApp());
}
