...
std::unique_ptr<std::vector<SMSRecord>> DBServiceAPI::SMSGetLimitOffset(sys::Service *serv, uint32_t offset,
                                                                        uint32_t limit)
{
...
std::unique_ptr<std::vector<ContactRecord>> DBServiceAPI::ContactGetByName(sys::Service *serv, UTF8 primaryName,
                                                                   ...
                                                                   ContactRecord &errPhone2, ContactRecord &speedDial)
{
    ...
    if (retName->size() != 0)
    {
        ...
        LOG_WARN("Name verification failed for [%s %s] owner ID: %d [%s %s]", rec.primaryName.c_str(),
                 rec.alternativeName.c_str());
    }
    else
    {
    ...
    {
        verified = speedDialError;
        LOG_WARN("Speed dial verification failed for number: [%d] owner ID: %d [%s %s]", rec.speeddial,
                 retSpeedDial->operator[](0).dbID, retSpeedDial->operator[](0).primaryName.c_str(),
    ...
    if (rec.numbers.size() > 0)
    {
        ...
        if (retPhone1->size() != 0)
        {
            verified = primaryNumberError;
            LOG_WARN("Phone number 1 verification failed for number: [%s] owner: ID: %d [%s %s]",
                     rec.numbers[0].numberE164.c_str(), retPhone1->operator[](0).dbID,
                     retPhone1->operator[](0).primaryName.c_str(), retPhone1->operator[](0).alternativeName.c_str());
    ...
    if (rec.numbers.size() > 1)
    {
        ...
        if (retPhone2->size() != 0)
        {
            verified = secondaryNumberError;
            LOG_WARN("Phone number 1 verification failed for number: [%s] owner: ID: %d [%s %s]",
                     rec.numbers[1].numberE164.c_str(), retPhone2->operator[](0).dbID,
                     retPhone2->operator[](0).primaryName.c_str(), retPhone2->operator[](0).alternativeName.c_str());
