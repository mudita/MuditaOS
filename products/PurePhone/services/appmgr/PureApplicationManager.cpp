...
#include "PureApplicationManager.hpp"
...
namespace app::manager
{
    ...
    PureApplicationManager::PureApplicationManager(const ApplicationName &serviceName,
        ...
        : ApplicationManager(serviceName, std::move(launchers), _rootApplicationName),
          ...
          simLockHandler(this),
          notificationsConfig{phoneModeObserver,
                                                                                         settings,
                                                                                         phoneLockHandler},
    ...
    sys::ReturnCodes PureApplicationManager::InitHandler()
    {
        ApplicationManager::InitHandler();
    ...
    void PureApplicationManager::processKeypadBacklightState(bsp::keypad_backlight::State keypadLightState)
    {
    ...
    void PureApplicationManager::startBackgroundApplications()
    {
    ...
    void PureApplicationManager::registerMessageHandlers()
    {
        ApplicationManager::registerMessageHandlers();
    ...
    void PureApplicationManager::handlePhoneModeChanged(sys::phone_modes::PhoneMode phoneMode)
    {
    ...
    void PureApplicationManager::changePhoneMode(sys::phone_modes::PhoneMode phoneMode, const ApplicationHandle *app)
    {
    ...
    void PureApplicationManager::handleTetheringChanged(sys::phone_modes::Tethering tethering)
    {
        notificationProvider.handle(tethering);
    }

    auto PureApplicationManager::handleAutoLockGetRequest([[maybe_unused]] GetAutoLockTimeoutRequest *request)
    ...
    auto PureApplicationManager::handleAutoLockSetRequest(SetAutoLockTimeoutRequest *request)
    -> std::shared_ptr<sys::ResponseMessage>
    {
    ...
    auto PureApplicationManager::handleDeveloperModeRequest(sys::Message *request) -> sys::MessagePointer
    {
    ...
    void PureApplicationManager::lockTimeChanged(std::string value)
    {
        if (value.empty()) {
    ...
    void PureApplicationManager::onPhoneLocked()
    {
        if (phoneLockHandler.isPhoneLocked()) {
            autoLockTimer.stop();
            return;
    ...
    auto PureApplicationManager::startApplication(ApplicationHandle &app) -> bool
    {
    ...
    auto PureApplicationManager::resolveHomeWindow() -> std::string
    {
        return phoneLockHandler.isPhoneLocked() ? gui::popup::window::phone_lock_window
                                                : gui::name::window::main_window;
    }

    auto PureApplicationManager::resolveHomeApplication() -> std::string
    {
        if (checkOnBoarding()) {
    ...
    auto PureApplicationManager::handleDBResponse(db::QueryResponse *msg) -> bool
    {
