...
void WorkerEvent::initProductHardware()
{
    ...
    bsp::torch::init(queues[static_cast<std::int32_t>(EventQueues::queueTorch)]->GetQueueHandle());
    bsp::light_sensor::init(queues[static_cast<std::int32_t>(EventQueues::queueLightSensor)]->GetQueueHandle());
...
bool WorkerEvent::handleMessage(std::uint32_t queueID)
{
    ...
    if (queueID == static_cast<std::uint32_t>(EventQueues::queueLightSensor)) {
        std::uint8_t notification;
        if (!queue->Dequeue(&notification, 0)) {
            return false;
        }
