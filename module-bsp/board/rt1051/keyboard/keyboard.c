...
static bsp_keyboard_event_callback_t    user_event_callback = NULL;
...
status_t bsp_keyboard_Init(bsp_keyboard_event_callback_t event)
{
	assert(event);
...
static void bsp_keyboard_worker(void* pvp)
{
	...
	while(1)
	{
		...
		if( ulNotificationValue & 0x01 )
		{
			...
			bsp_i2c_Receive(BOARD_GetI2CInstance(),TCA8418_I2C_ADDRESS,REG_KEY_LCK_EC,(uint8_t*)&retval,1);
			...
			for(i=0;i<events_count;i++){
				uint8_t key = 0;
				uint8_t rel_pres = 0;
				bsp_i2c_Receive(BOARD_GetI2CInstance(),TCA8418_I2C_ADDRESS,REG_KEY_EVENT_A,(uint8_t*)&retval,1);
				...
				user_event_callback(rel_pres,key);
			}
	    ...
	    {
            user_event_callback(KEY_PRESS, KeyboardFnRight);
	    }

	    if (ulNotificationValue & 0x04)
	    {
            user_event_callback(KEY_RELEASE, KeyboardFnRight);
	    }
	}
}
...
static void s_bsp_keyboard_right_functional_check_timer_cb(TimerHandle_t timer)
{
    uint8_t right_functional_current_state = GPIO_PinRead(BOARD_KEYBOARD_RF_BUTTON_PORT, BOARD_KEYBOARD_RF_BUTTON_PIN);
    xTimerStop(timer, 0);
