                                           ...
                                           uint32_t *length)
{
    ...
    if ((setup->bmRequestType & USB_REQUEST_TYPE_RECIPIENT_MASK) == USB_REQUEST_TYPE_RECIPIENT_DEVICE) {
        ...
        {
#endif
            error = USB_DeviceGetStatus(
                classHandle->handle, kUSB_DeviceStatusDevice, &classHandle->standardTranscationBuffer);
                                               ...
                                               uint32_t *length)
{
    ...
    if (USB_DESCRIPTOR_TYPE_DEVICE == descriptorType) {
        ...
        error = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetDeviceDescriptor, &commonDescriptor.deviceDescriptor);
    }
        ...
        error                                                  = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetConfigurationDescriptor, &commonDescriptor.configurationDescriptor);
    }
        ...
        error                                         = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetStringDescriptor, &commonDescriptor.stringDescriptor);
    }
        ...
        error                                          = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetHidDescriptor, &commonDescriptor.hidDescriptor);
    }
        ...
        error                                                = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetHidReportDescriptor, &commonDescriptor.hidReportDescriptor);
    }
        ...
        error                                                  = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetHidPhysicalDescriptor, &commonDescriptor.hidPhysicalDescriptor);
    }
#endif
        ...
        /* Get the device descriptor */
        error = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetDeviceQualifierDescriptor, &commonDescriptor.deviceDescriptor);
    }
#endif
        ...
        commonDescriptor.configurationDescriptor.configuration = descriptorIndex;
        error                                                  = USB_DeviceClassCallback(
            classHandle->handle, kUSB_DeviceEventGetBOSDescriptor, &commonDescriptor.configurationDescriptor);
    }
#endif
    else {
    }
                                                  ...
                                                  uint32_t *length)
{
    ...
    *buffer = (uint8_t *)&classHandle->standardTranscationBuffer;
    return USB_DeviceClassCallback(
        classHandle->handle, kUSB_DeviceEventGetConfiguration, &classHandle->standardTranscationBuffer);
}
                                              ...
                                              uint32_t *length)
{
    ...
    *buffer                                = (uint8_t *)&classHandle->standardTranscationBuffer;
     ...
     * application. */
    error = USB_DeviceClassCallback(
        classHandle->handle, kUSB_DeviceEventGetInterface, &classHandle->standardTranscationBuffer);
    classHandle->standardTranscationBuffer = USB_SHORT_TO_LITTLE_ENDIAN(classHandle->standardTranscationBuffer);
    return error;
}
                                              ...
                                              uint32_t *length)
{
    ...
    USB_DeviceClassEvent(
        classHandle->handle, kUSB_DeviceClassEventSetInterface, &classHandle->standardTranscationBuffer);
    ...
    return USB_DeviceClassCallback(
        classHandle->handle, kUSB_DeviceEventSetInterface, &classHandle->standardTranscationBuffer);
}
                                       ...
                                       void *callbackParam)
{
    ...
    if (message->isSetup) {
        if ((USB_SETUP_PACKET_SIZE != message->length) || (NULL == message->buffer)) {
            ...
            USB_DeviceDeinitEndpoint(
                handle, USB_CONTROL_ENDPOINT | (USB_OUT << USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT));
        ...
        if ((deviceSetup->bmRequestType & USB_REQUEST_TYPE_TYPE_MASK) == USB_REQUEST_TYPE_TYPE_STANDARD) {
            ...
            if ((deviceSetup->wLength) &&
                ...
                else if ((deviceSetup->wLength) && ((deviceSetup->bmRequestType & USB_REQUEST_TYPE_TYPE_VENDOR) ==
                ...
                else if (((deviceSetup->bmRequestType & USB_REQUEST_TYPE_TYPE_VENDOR) ==
                          USB_REQUEST_TYPE_TYPE_VENDOR)) {
                    /* Get data buffer to response the host. */
                    usb_device_control_request_struct_t controlRequest;
                    controlRequest.buffer  = (uint8_t *)NULL;
        ...
        error = USB_DeviceControlCallbackFeedback(
            handle, deviceSetup, error, kUSB_DeviceControlPipeSetupStage, &buffer, &length);
    }
    ...
    (defined(USB_DEVICE_CONFIG_USB20_TEST_MODE) && (USB_DEVICE_CONFIG_USB20_TEST_MODE > 0U))
        ...
        /* Send the response to the host. */
        error = USB_DeviceControlCallbackFeedback(
            handle, deviceSetup, error, kUSB_DeviceControlPipeDataStage, &buffer, &length);
    }
    else {
    }
    return error;
