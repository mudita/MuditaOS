...
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) usb_device_mtp_struct_t g_mtpHandle[USB_DEVICE_CONFIG_MTP];
                                             ...
                                             void *callbackParam)
{
    ...
    if ((NULL != mtpHandle->configStruct) && (mtpHandle->configStruct->classCallback)) {
        ...
        error = mtpHandle->configStruct->classCallback(
            kUSB_DeviceMtpEventSendResponse, message, mtpHandle->configStruct->classCalbackArg);
    }
    return error;
}
...
usb_status_t USB_DeviceClassMtpEndpointsDeinit(usb_device_mtp_struct_t *mtpHandle)
{
    ...
    for (uint32_t count = 0; count < mtpHandle->interfaceHandle->endpointList.count; count++) {
        error = USB_DeviceDeinitEndpoint(mtpHandle->handle,
...
static usb_status_t handler_DeviceClassEventClassRequest(usb_device_mtp_struct_t *mtp, void *param)
{
    ...
    if (param) {
        ...
        if (request->setup) {
            usb_echo("[MTP] Control request: [bmRequestType = 0x%x, bRequest = 0x%x, wValue = 0x%x, wIndex = 0x%x, "
                     "wLength = 0x%x]\n",
        ...
        case MTP_CLASS_REQUEST_CANCEL:
            error = mtp->configStruct->classCallback(
                kUSB_DeviceMtpEventCancelTransaction, controlRequest, mtp->configStruct->classCalbackArg);
            break;
        case MTP_CLASS_REQUEST_DEVICE_STATUS:
            error = mtp->configStruct->classCallback(
                kUSB_DeviceMtpEventRequestDeviceStatus, controlRequest, mtp->configStruct->classCalbackArg);
            break;
