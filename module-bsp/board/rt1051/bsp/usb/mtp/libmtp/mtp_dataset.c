...
uint32_t serialize_device_info(const mtp_device_info_t *info, uint8_t *data)
{
    ...
    length += put_array(
        data + length, MTP_SUPPORTED_OPERATIONS, sizeof(MTP_SUPPORTED_OPERATIONS) / sizeof(uint16_t), sizeof(uint16_t));
    length += put_array(
        data + length, MTP_SUPPORTED_EVENTS, sizeof(MTP_SUPPORTED_EVENTS) / sizeof(uint16_t), sizeof(uint16_t));
    length += put_array(data + length,
                        MTP_SUPPORTED_DEVICE_PROPERTIES,
                        sizeof(MTP_SUPPORTED_DEVICE_PROPERTIES) / sizeof(uint16_t),
                        sizeof(uint16_t));
    length += put_array(data + length,
                        MTP_SUPPORTED_CAPTURE_FORMATS,
                        sizeof(MTP_SUPPORTED_CAPTURE_FORMATS) / sizeof(uint16_t),
                        sizeof(uint16_t));
    length += put_array(data + length,
                        MTP_SUPPORTED_PLAYBACK_FORMATS,
