...
{
    static const dbg_map_entry_t ops[] = {{"MTP_OPERATION_GET_DEVICE_INFO", 0x1001},
...
{
    static const dbg_map_entry_t events[] = {{"MTP_EVENT_UNDEFINED", 0x4000},
...
{
    static const dbg_map_entry_t results[] = {{"MTP_RESPONSE_UNDEFINED", 0x2000},
...
{
    assert(mtp && info);
    if (!info || !info->manufacturer || !info->model || !info->serial || !info->version ||
...
static uint16_t operation_get_object_handles(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    ...
    *ptr++ = count;
    ...
    if (count) {
        uint32_t available =
            (mtp->buf_size - MTP_CONTAINER_HEADER_SIZE) / sizeof(uint32_t) - 1; // -1 beacuse 4 bytes for array length
...
static uint16_t operation_get_object_props_supported(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint8_t *payload     = (uint8_t *)mtp->cntr->payload;
...
static uint16_t operation_get_object_prop_desc(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error;
    uint8_t *payload     = (uint8_t *)mtp->cntr->payload;
...
static uint16_t operation_get_object_prop_value(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error;
    uint8_t *payload       = (uint8_t *)mtp->cntr->payload;
    uint32_t obj_handle    = request->parameter[0];
...
static uint16_t operation_set_object_prop_value(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error;
    uint32_t obj_handle    = request->parameter[0];
    uint16_t prop_code     = request->parameter[1];
...
static uint16_t operation_get_object(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error;
    uint32_t obj_handle = request->parameter[0];
    ...
    int data_read = mtp->storage.api->read(mtp->storage.api_arg, mtp->cntr->payload, empty_space);
    if (data_read < 0) {
...
static uint16_t operation_delete_object(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error;
    uint32_t obj_handle = request->parameter[0];
...
static uint16_t operation_send_object_info(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error;
...
static uint16_t operation_send_object(mtp_responder_t *mtp, const mtp_op_cntr_t *request)
{
    uint16_t error = 0;
