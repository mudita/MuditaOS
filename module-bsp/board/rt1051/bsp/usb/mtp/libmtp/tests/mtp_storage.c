...
static const mtp_storage_properties_t props = {.type        = MTP_STORAGE_FIXED_ROM,
...
Ensure(mtp_storage, list_serialize_empty_array_if_no_files)
{
    ...
    expect(mock_find_first, when(parent, is_equal_to(0)), will_return(0));
...
{
    uint8_t expected[] = {0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x81, 0x00, 0x00, 0x00, 0x82};
    uint32_t COUNT     = 2;
    ...
    expect(mock_find_next, will_return(0x82000000));
    expect(mock_find_next, will_return(0));
...
{
    expect(mock_get_properties, will_return(&props));

    expect(mock_free_space, will_return(1024));
    ...
    expect(put_string, when(text, is_equal_to(NULL)), will_return(1)); /* Volume Identifier */
...
{
    const uint8_t expected[] = {0x01, 0x00, 0x00, 0x00, 0xef, 0xbe, 0xcd, 0xab};
...
Ensure(mtp_storage, object_info)
{
    ...
    expect(
        put_string, when(text, is_equal_to_contents_of("file.name.txt", 13)), will_return(1 + 14 * 2)); /* Filename */
    expect(put_date, when(time, is_equal_to(1580322989)), will_return(1 + 16 * 2)); /* Date created */
    expect(put_date, when(time, is_equal_to(1580323100)), will_return(1 + 16 * 2)); /* Date modified */
    expect(put_string, when(text, is_equal_to(NULL)), will_return(3));
