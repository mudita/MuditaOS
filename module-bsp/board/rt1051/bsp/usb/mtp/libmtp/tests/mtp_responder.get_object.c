...
Ensure(get_object, returns_error_when_file_doesnt_exists)
{
    ...
    expect(mock_stat, when(info, is_not_equal_to(NULL)), will_return(-1));
    error           = mtp_responder_handle_request(mtp, request, sizeof(request));
    given_data_size = mtp_responder_get_data(mtp);
    assert_that(error, is_equal_to(MTP_RESPONSE_INVALID_OBJECT_HANDLE));
    assert_that(given_data_size, is_equal_to(0));
...
Ensure(get_object, returns_error_when_unable_open_the_file)
{
    ...
    expect(mock_stat,
           ...
           will_return(0));
    expect(mock_open, will_return(-1));
    error           = mtp_responder_handle_request(mtp, request, sizeof(request));
    given_data_size = mtp_responder_get_data(mtp);
    assert_that(error, is_equal_to(MTP_RESPONSE_INVALID_OBJECT_HANDLE));
    assert_that(given_data_size, is_equal_to(0));
...
Ensure(get_object, returns_error_when_read_from_file_failed)
{
    ...
    expect(mock_read, when(count, is_greater_than(35)), will_return(-1));
    expect(mock_close);
    error           = mtp_responder_handle_request(mtp, request, sizeof(request));
    given_data_size = mtp_responder_get_data(mtp);
...
Ensure(get_object, returns_data_when_file_content_fits_in_one_frame)
{
    ...
    expect(mock_stat,
           ...
           will_return(0));
    expect(mock_open, will_return(0));
    expect(mock_read, when(count, is_equal_to(500)), will_return(200));
    error           = mtp_responder_handle_request(mtp, request, sizeof(request));
    given_data_size = mtp_responder_get_data(mtp);
...
Ensure(get_object, returns_data_when_file_content_exceeds_one_frame)
{
    ...
    expect(mock_stat,
           ...
           will_return(0));
    expect(mock_open, will_return(0));
    expect(mock_read, when(count, is_equal_to(500)), will_return(500));
    expect(mock_read, when(count, is_equal_to(512)), will_return(512));
    expect(mock_read, when(count, is_equal_to(512)), will_return(12));
    expect(mock_close);
