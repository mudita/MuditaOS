...
int deserialize_object_info(const uint8_t *data, size_t length, mtp_object_info_t *info)
{
    ...
    info->storage_id   = *(uint32_t *)ptr;
    ptr += 4;
    info->format_code = *(uint16_t *)ptr;
    ptr += 2;
    info->protection = *(uint16_t *)ptr;
    ptr += 2;
    info->size = *(uint32_t *)ptr;
    ptr += 4;
    ptr += 1 * sizeof(uint16_t);
    ptr += 6 * sizeof(uint32_t);
    info->parent = *(uint32_t *)ptr;
    ptr += 4;
    info->association_type = *(uint16_t *)ptr;
    ptr += 2;
    info->association_desc = *(uint32_t *)ptr;
    ptr += 4;
    ptr += sizeof(uint32_t);
