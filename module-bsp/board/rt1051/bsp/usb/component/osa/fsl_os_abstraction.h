...
#define OSA_TASK_DEFINE(name, priority, instances, stackSz, useFloat)                                                  \
    ...
    osThreadStackDef(name, stackSz, instances) osa_task_def_t os_thread_def_##name = {(name),                          \
                                                                                      (priority),                      \
                                                                                      (instances),                     \
                                                                                      (stackSz),                       \
...
#else
#define OSA_TASK_DEFINE(name, priority, instances, stackSz, useFloat)                                                  \
    osa_task_def_t os_thread_def_##name = {                                                                            \
        (name), (priority), (instances), (stackSz), NULL, NULL, (uint8_t *)#name, (useFloat)}
#endif
...
#define OSA_EXIT_CRITICAL() OSA_ExitCritical(osaCurrentSr)
     ...
     *
     * This function  allocates memory for and initializes a message queue. Message queue elements are hardcoded as
     * void*.
     *
