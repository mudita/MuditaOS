...
namespace drivers{
    ...
    int32_t RT1051DriverSAI::StartInTransfer(const drivers::TransferParams &rx) {
        ...
        SAI_TransferRxCreateHandleEDMA(base, &rxHandle[enum_integer(instances)], rxAudioCallback, this,
                                       reinterpret_cast<edma_handle_t*>(dma->GetHandle(parameters.rxDMAChannel)));

        SAI_TransferRxSetFormatEDMA(base, &rxHandle[enum_integer(instances)], &sai_format, parameters.masterClock,
                                    parameters.masterClock);
    ...
    int32_t RT1051DriverSAI::StartOutTransfer(const drivers::TransferParams &tx) {
        ...
        SAI_TransferTxCreateHandleEDMA(base, &txHandle[enum_integer(instances)], txAudioCallback, this, reinterpret_cast<edma_handle_t*>(dma->GetHandle(parameters.txDMAChannel)));

        SAI_TransferTxSetFormatEDMA(base, &txHandle[enum_integer(instances)], &sai_format, parameters.masterClock,
                                    parameters.masterClock);
