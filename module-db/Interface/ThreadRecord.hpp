...
class ThreadRecordInterface : public RecordInterface<ThreadRecord, ThreadRecordField>
{
  public:
    ...
    std::unique_ptr<db::QueryResult> getQuery(const db::Query *query) override;
  ...
  private:
    ...
    std::unique_ptr<db::query::SMSSearchResult> getQueryImpl(const db::query::SMSSearch *query);
};
