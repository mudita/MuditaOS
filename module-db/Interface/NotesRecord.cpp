...
bool NotesRecordInterface::Add(const NotesRecord &rec) {
    ...
    uint32_t contactID = 0;
      ...
      // Create SMS
    notesDB->notes.Add(NotesTableRow{
            .date=rec.date,
            .snippet=rec.snippet,
			.path=rec.path
    });
...
std::unique_ptr<std::vector<NotesRecord>> NotesRecordInterface::GetLimitOffsetByField(uint32_t offset, uint32_t limit,
...
bool NotesRecordInterface::Update(const NotesRecord &rec) {
    ...
    notesDB->notes.Update(NotesTableRow{
        .ID=rec.ID,
        .date=rec.date,
        .snippet=rec.snippet,
		.path=rec.path
