...
AlarmEventsTableRow::AlarmEventsTableRow(uint32_t id,
    ...
    : EventsTableRow{id, std::move(name), startDate, endDate, duration, isAllDay, rrule}, musicTone{musicTone},
...
std::vector<AlarmEventsTableRow> AlarmEventsTable::getLimitOffset(uint32_t offset, uint32_t limit)
{
    ...
    std::vector<AlarmEventsTableRow> ret;
                                                                   ...
                                                                   uint32_t limit)
{
    ...
    do {
        ret.push_back(AlarmEventsTableRow(*retQuery));
    } while (retQuery->nextRow());
...
std::string AlarmEventsTable::getFieldName(AlarmEventsTableFields field)
{
    ...
    default:
        throw std::invalid_argument{"Invalid event table field passed."};
    }
}
