...
bool ContactsRingtonesTable::Update(ContactsRingtonesTableRow entry) {
    return db->Execute(
            "UPDATE contact_ringtones SET contact_id = %lu, asset_path = '%s' WHERE _id=%lu;",
            entry.contactID,
            entry.assetPath.c_str(),
            entry.ID
    );
...
ContactsRingtonesTableRow ContactsRingtonesTable::GetByID(uint32_t id) {
    ...
    return ContactsRingtonesTableRow{(*retQuery)[0].GetUInt32(),  // ID
...
std::vector<ContactsRingtonesTableRow> ContactsRingtonesTable::GetLimitOffset(uint32_t offset, uint32_t limit) {
    auto retQuery = db->Query("SELECT * from contact_ringtones ORDER BY contact_id LIMIT %lu OFFSET %lu;",
                              limit,
                              offset);
    ...
    do {
        ret.push_back(ContactsRingtonesTableRow{(*retQuery)[0].GetUInt32(),  // ID
                                                (*retQuery)[1].GetUInt32(),    // contactID
                                                (*retQuery)[2].GetString()     // assetPath
        });
