...
{
    return db->Execute("insert or ignore into contact_ringtones (contact_id, asset_path ) VALUES (%lu, '%s');", entry.contactID, entry.assetPath.c_str());
}
...
{
    return db->Execute("UPDATE contact_ringtones SET contact_id = %lu, asset_path = '%s' WHERE _id=%lu;", entry.contactID, entry.assetPath.c_str(), entry.ID);
}
...
{
    auto retQuery = db->Query("SELECT * from contact_ringtones ORDER BY contact_id LIMIT %lu OFFSET %lu;", limit, offset);
                                                                                     ...
                                                                                     const char *str)
{
    ...
    auto retQuery = db->Query("SELECT * from contact_ringtones WHERE %s='%s' ORDER BY contact_id LIMIT %lu OFFSET %lu;", fieldName.c_str(), str, limit, offset);
