...
bool ContactsTable::Add(ContactsTableRow entry)
{
    return db->Execute("insert or ignore into contacts (name_id, numbers_id, ring_id, address_ids, type, whitelist, "
                       "blacklist, favourites, speeddial ) VALUES (%lu, '%s', %lu, '%s', %lu, %lu, %lu, %lu, %lu);",
                       entry.nameID, entry.numbersID.c_str(), entry.ringID, entry.addressIDs.c_str(), entry.type,
                       entry.isOnWhitelist, entry.isOnBlacklist, entry.isOnFavourites, entry.speedDial);
}
...
bool ContactsTable::Update(ContactsTableRow entry)
{
    return db->Execute("UPDATE contacts SET name_id = %lu, numbers_id = '%s' ,ring_id = %lu, address_ids = '%s', type "
                       "= %lu, whitelist = %lu, blacklist = %lu, favourites = %lu, speeddial = %lu WHERE _id=%lu;",
                       entry.nameID, entry.numbersID.c_str(), entry.ringID, entry.addressIDs.c_str(), entry.type,
                       entry.isOnWhitelist, entry.isOnBlacklist, entry.isOnFavourites, entry.speedDial, entry.ID);
}
...
std::vector<ContactsTableRow> ContactsTable::Search(const std::string primaryName, const std::string alternativeName,
                                                    const std::string number)
{
    ...
    {
        LOG_INFO("need at least one field to search all are empty");
        return (ret);
    }

    std::string q = "select t1.* from contacts t1 inner join contact_name "
                    "t2 "
    ...
    LOG_INFO("constructed query: \"%s\"", q.c_str());
...
std::vector<ContactsTableRow> ContactsTable::GetLimitOffsetByField(uint32_t offset, uint32_t limit,
                                                                   ContactTableFields field, const char *str)
{
    ...
    auto retQuery = db->Query("SELECT * from contacts WHERE %s='%s' ORDER BY name_id LIMIT %lu OFFSET %lu;",
                              fieldName.c_str(), str, limit, offset);
