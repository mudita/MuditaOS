...
namespace app
{
    ...
    void ApplicationBellSettings::createUserInterface()
    {
        ...
        windowsFactory.attach(gui::window::name::bellSettingsTimeUnits, [](Application *app, const std::string &name) {
            return std::make_unique<gui::BellSettingsTimeUnitsWindow>(app);
        });

        // Dummy views for not implemented applications
        windowsFactory.attach(gui::window::name::bellSettingsDialog, [](Application *app, const std::string &name) {
