    ...
    template <class T> class DatabaseModel
    {
      ...
      public:
                                   ...
                                   uint32_t count)
        {
            ...
            //                    auto ii           = i + (pageSize * (page - currentPage) + pageSize) + offset %
            //                    pageSize; this->records[ii] = nullptr; this->records[i + (pageSize * (page -
            //                    currentPage) + pageSize) + offset % pageSize] =
            //                        std::make_shared<T>(records.get()->operator[](i));
            //                    // this->records[i+(pageSize*(page-currentPage) + pageSize)+offset% limit] =
        ...
        bool setRecordsCount(int count)
        {
            ...
            //            // if there are any records request blocks of data from database using recordsLocalCount and
            //            requestSize. if (recordsCount) {
            //
        ...
        std::shared_ptr<T> getRecord(int index, bool download = true)
        {
            ...
            //            if ((download == false) && (index >= firstIndex - pageSize) && (index < firstIndex + 2 *
            //            pageSize)) {
            //                return records[pageSize + index - firstIndex];
            //            }
