...
void PhonebookModel::requestRecordsCount()
{
    ...
    if (static_cast<uint32_t>(recordsCount) > pageSize) requestRecords(pageSize, pageSize);
}
...
void PhonebookModel::requestRecords(const uint32_t offset, const uint32_t limit)
{
    ...
    if (offset < favouriteCount)
    {
        ...
        if (offset + limit < favouriteCount) { DBServiceAPI::ContactGetLimitOffset(application, offset, limit, true); }
        else
        {
                                   ...
                                   bool favourite)
{
    ...
    if (favourite) DatabaseModel::updateRecords(std::move(records), offset, limit, count);
    else
...
gui::ListItem *PhonebookModel::getItem(int index, int firstElement, int prevIndex, uint32_t count, int remaining, bool topDown)
{
    ...
    bool download = false;
    if (index > firstIndex + pageSize / 2) download = true;
    if (index < firstIndex - pageSize / 2) download = true;
    ...
    if (topDown)
    {
        ...
        if (static_cast<uint32_t>(index) < favouriteCount)
        {
            ...
            if ((index == firstElement) && (index != prevIndex)) { item->setValue(utils::localize.get("app_phonebook_list_favourites")); }
            else
            {
    ...
    else
    {
        ...
        {
            gui::PhonebookItem *item = new gui::PhonebookItem(this);
            if (remaining == 0) { item->setValue(utils::localize.get("app_phonebook_list_favourites")); }
            else
            {
                item->markFavourite(true);
                item->setContact(contact);
        ...
        else
        {
            ...
            if ((static_cast<uint32_t>(index) == favouriteCount - 1) && (index != prevIndex)) { item->setValue(contact->alternativeName.substr(0, 1)); }
            else
            {
                std::shared_ptr<ContactRecord> prevContact = getRecord(prevIndex, false);
                if (remaining == 0)
