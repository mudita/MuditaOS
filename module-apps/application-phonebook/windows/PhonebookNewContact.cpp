...
void PhonebookNewContact::buildInterface()
{
    ...
    if (contact) { setTitle(utils::localize.get("app_phonebook_contact_edit")); }
    else
    {
    ...
    for (uint32_t i = 0; i < 5; i++)
    {
        ...
        if ((i == 2) || (i == 3))
        {
            page1.text[i]->focusChangedCallback = [=](gui::Item &item) {
                if (item.focus)
                {
                    page1.text[i]->setFont(style::footer::font::bold);
    ...
    page2.favValue->focusChangedCallback = [=](gui::Item &item) {
        if (item.focus) { bottomBar->setText(BottomBar::Side::CENTER, "SELECT"); }
        else
        {
            bottomBar->setText(BottomBar::Side::CENTER, utils::localize.get("app_phonebook_save"));
        }
...
void PhonebookNewContact::setContactData()
{
    if (contact)
    {
        ...
        if (contact->numbers.size() > 0) { page1.text[2]->setText(contact->numbers[0].numberE164); }
        if (contact->numbers.size() > 1) { page1.text[3]->setText(contact->numbers[1].numberE164); }
...
bool PhonebookNewContact::onInput(const InputEvent &inputEvent)
{
    ...
    if (inputEvent.keyCode == KeyCode::KEY_ENTER)
    {
        ...
        if (focusItem == page2.favValue) { return true; }
        else
        {
            LOG_INFO("PhonebookNewContact::onInput time to save the contact");
        }
