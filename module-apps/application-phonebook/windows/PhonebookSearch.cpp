                                 ...
                                 const bool lineMode = false)
{
    ...
    // addItem(l);
    if (parentPage) parentPage->push_back(l);
...
void PhonebookSearch::buildInterface()
{
    ...
    topSeparatorLabel = addLabel(nullptr, 0, 104, 480, 1, "", style::window::font::small, RectangleEdgeFlags::GUI_RECT_EDGE_BOTTOM);
    ...
    informationLabel->setAlignment(Alignment(Alignment::ALIGN_HORIZONTAL_CENTER, Alignment::ALIGN_VERTICAL_BOTTOM));
    ...
    inputField->setAlignment(Alignment(Alignment::ALIGN_HORIZONTAL_LEFT, Alignment::ALIGN_VERTICAL_BOTTOM));
...
bool PhonebookSearch::onInput(const InputEvent &inputEvent)
{
    ...
    if (inputEvent.keyCode == KeyCode::KEY_ENTER)
    {
        ...
        if (contents.size() > 0)
        {
            if (utils::is_number(contents) || contents.find('+') != std::string::npos) { number = contents; }
            ...
            else
            {
                ...
                data->setResults(std::move(searchResults));
