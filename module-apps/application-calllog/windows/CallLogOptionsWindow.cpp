...
namespace gui
{
    ...
    CallLogOptionsWindow::CallLogOptionsWindow(app::Application *app)
        : AppWindow(app, calllog::settings::OptionsWindowStr)
    {
        buildInterface();
    }
    ...
    void CallLogOptionsWindow::rebuild()
    {
        ...
        std::vector<gui::Item *>::iterator it =
            std::find_if(options.begin(), options.end(), [=](auto val) { return val == getFocusItem(); });
        if (it != options.end()) {
    ...
    gui::Item *CallLogOptionsWindow::addOptionLabel(const std::string &text,
