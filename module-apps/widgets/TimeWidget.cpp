...
namespace gui
{
    ...
    void TimeWidget::applyInputCallbacks()
    {
        ...
        onSaveCallback = [&](std::shared_ptr<EventsRecord> record) -> bool {
            ...
            if (type == Type::End) {
                record->date_till = date + hours + minutes;
            }
            else if (type == Type::Start) {
                record->date_from = date + hours + minutes;
            }
            return true;
        };
        ...
        if (!mode24H) {
            ...
            onLoadCallback = [&](std::shared_ptr<EventsRecord> event) {
                if (type == Type::Start) {
                    ...
                    hourInput->setText(TimePointToLocalizedHourString(event->date_from));
                    ...
                    auto hours = TimePointToLocalizedHourString(event->date_till);
                    hourInput->setText(hours);
                    minuteInput->setText(TimePointToMinutesString(event->date_till));
                    if (date::is_am(std::chrono::hours{std::stoi(hours)})) {
                        mode12hInput->setText(timeConstants::before_noon);
                    }
                    else {
                        mode12hInput->setText(timeConstants::after_noon);
            ...
            onLoadCallback = [&](std::shared_ptr<EventsRecord> event) {
                ...
                if (type == Type::Start) {
                    ...
                    hourInput->setText(TimePointToLocalizedHourString(event->date_till));
                    minuteInput->setText(TimePointToMinutesString(event->date_till));
                }
            };
        }
    ...
    TimePoint TimeWidget::calculateEventTime(calendar::YearMonthDay date,
