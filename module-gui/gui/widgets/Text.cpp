...
Text::TextLine::TextLine(const UTF8 &text, uint32_t startIndex, uint32_t endIndex, Text::LineEndType endType,
...
Text::Text(Item *parent, const uint32_t &x, const uint32_t &y, const uint32_t &w, const uint32_t &h, const UTF8 &text,
           ExpandMode expandMode, TextType textType)
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (spaceIndex == tmpText.length() - 1)
                    {
                        ...
                        index += charCount;
                        textLines.push_back(new TextLine(tmpText.substr(0, tmpText.length() - 1), startIndex, endIndex,
                                                         lineEndType, spaceConsumed - spaceWidth));
                        //						LOG_INFO("Text Input Line: [%s]", textLines.back()->text.c_str());
                    }
                    else
                    {
                        ...
                        index += spaceIndex + 1;
                        textLines.push_back(new TextLine(tmpText.substr(0, spaceIndex), startIndex, endIndex,
                                                         lineEndType,
...
void Text::reworkLines(std::list<TextLine *>::iterator it)
{
    ...
    while (it != textLines.end())
    {
        ...
        if (splitFlag)
        {
            ...
            itNext = it;
            itNext++;
            textLines.insert(itNext, new TextLine(remainingText, 0, remainingText.length(), (*it)->endType,
                                                  font->getPixelWidth(remainingText)));
...
bool Text::onInput(const InputEvent &inputEvent)
{
    ...
    bool res = false;
    if ((inputEvent.keyCode == KeyCode::KEY_LEFT) || (inputEvent.keyCode == KeyCode::KEY_RIGHT) ||
...
bool Text::handleEnter()
{
    ...
    ++itNext;
    textLines.insert(
        itNext, new TextLine(remainingText, 0, remainingText.length(), endType, font->getPixelWidth(remainingText)));
    cursorRow++;
...
void Text::recalculateDrawParams()
{
    ...
    {
        gui::Label *label = new gui::Label(this, margins.left, startY, widgetArea.w - margins.left - margins.right,
