...
namespace gui
{
    ...
    class ListItemProvider_old;

    /// fixed size and application Vertical ListView_old to be used with ListItemProvider_old and DatabaseModel only.
    /// For simple vertical list with no external model use VBox
    class ListView_old : public Rect
    {
      protected:
        ...
        ListItemProvider_old *provider = nullptr;
      ...
      public:
        ...
        ListView_old();
        ListView_old(Item *parent, uint32_t x, uint32_t y, uint32_t w, uint32_t h);
        virtual ~ListView_old();
