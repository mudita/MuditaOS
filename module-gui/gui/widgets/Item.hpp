...
public:
  ...
  auto area(Area which = Area::Normal) -> BoundingBox &
  {
    ...
    /// callback functors are meant to emulate signal <-> slot actions where you bind element instance to in code defined lambda function
    /// all Items have functions coresponding to callback
    ...
    /// @attention all callbacks return true if handled, return true means end of event processing, i.e. if you handle enter in inputCallback, then
    /// activatedCallback shouldn't be called.
    /// @{
    ...
    /// called when `this` Item was pressed with enter (middle key on phone action keys), used for callback input handling
    /// calls activatedCallback
