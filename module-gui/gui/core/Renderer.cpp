            ...
            if (glyph_found != font->glyphs.end()) {
                ...
                if (i == 0) {
                    ...
                    int32_t kernValue = font->getKerning(idLast, idCurrent);
                    {
                        ...
                        //#endif
                        drawChar(
                            drawCtx, wgtX + posX + glyph->xoffset + kernValue, wgtY + posY, font, glyph, cmd->color);
    ...
    void Renderer::drawImage(Context *ctx, CommandImage *cmd)
    {
        ...
        if (imageMap->getType() == gui::ImageMap::Type::PIXMAP) {
            ...
            if (drawHeight > drawCtx->getH()) {
                LOG_WARN("image %s height exceeds context (%d > %d). truncating",
                         pixMap->getName().c_str(),
                         imageHeight,
                         drawCtx->getH());
            ...
            if (drawWidth > drawCtx->getW()) {
                LOG_WARN("image %s width exceeds context (%d > %d). truncating",
                         pixMap->getName().c_str(),
                         imageWidth,
                         drawCtx->getW());
