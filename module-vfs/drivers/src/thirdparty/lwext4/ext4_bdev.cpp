...
namespace purefs::fs::drivers::ext4::internal
{
        ...
        namespace io
        {
            ...
            struct context
            {
                context(std::shared_ptr<blkdev::disk_manager> diskmm, blkdev::disk_fd _disk_h)
                    : disk(diskmm), disk_h(std::move(_disk_h))
                {}
            ...
            int write(struct ext4_blockdev *bdev, const void *buf, std::uint64_t blk_id, std::uint32_t blk_cnt)
            {
            ...
            int read(struct ext4_blockdev *bdev, void *buf, std::uint64_t blk_id, std::uint32_t blk_cnt)
            {
                auto ctx = reinterpret_cast<context *>(bdev->bdif->p_user);
                if (!ctx) {
                    return -EIO;
    ...
    std::pair<ext4_blockdev *, int> append_volume(std::shared_ptr<blkdev::disk_manager> diskmm, blkdev::disk_fd diskh)
    {
        ...
        std::memset(&cfg->bdev, 0, sizeof(cfg->bdev));
        std::memset(&cfg->ifc, 0, sizeof(cfg->ifc));
        cfg->ifc.open         = io::open;
        cfg->ifc.bread        = io::read;
        cfg->ifc.bwrite       = io::write;
        cfg->ifc.close        = io::close;
        cfg->buf              = std::make_unique<std::uint8_t[]>(sect_size);
        ...
        cfg->bdev.part_size   = std::uint64_t(sect_size) * sect_count;
