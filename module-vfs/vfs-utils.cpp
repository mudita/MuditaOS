...
bool vfs::verifyCRC(const fs::path filePath)
{
    ...
    if (fp.get() != nullptr) {
        if (::vfs.fread(crcBuf.get(), 1, purefs::buffer::crc_char_size, fp.get()) != purefs::buffer::crc_char_size) {
            LOG_ERROR("verifyCRC fread on %s returned different size then %d", crcFilePath.c_str(), purefs::buffer::crc_char_size);
            return false;
        }

        const unsigned long crc32Read = strtoull(crcBuf.get(), nullptr, purefs::buffer::crc_radix);

        LOG_INFO("verifyCRC read %s string:\"%s\" hex:%08X", filePath.c_str(), crcBuf.get(), crc32Read);
...
bool vfs::updateFileCRC32(const fs::path &file)
{
    ...
    if (fp.get() != nullptr) {
        ...
        if (fileCRC32 != 0) {
            if ( (written = sprintf(crc32Buf.get(), "%08X", fileCRC32)) != purefs::buffer::crc_char_size) {
                LOG_INFO("updateFileCRC32 can't prepare string for crc32, sprintf returned %d instead of %d", written, purefs::buffer::crc_char_size);
                return false;
            }
            ...
            if (fpCRC32.get() != nullptr) {
                if (fwrite(crc32Buf.get(), 1, purefs::buffer::crc_char_size, fpCRC32.get()) == purefs::buffer::crc_char_size) {
