...
{
    unsigned char buf[purefs::buffer::crc_buf];
    size_t bufLen;

    *outCrc32 = 0;

    while (!::vfs.eof(file)) {
        bufLen = ::vfs.fread(buf, 1, purefs::buffer::crc_buf, file);
        if (bufLen <= 0)
            break;
...
{
    char crcBuf[purefs::buffer::crc_char_size] = {0};
    fs::path crcFilePath(filePath);
    crcFilePath += purefs::extension::crc32;

    vfs::FILE *fp = ::vfs.fopen(crcFilePath.c_str(), "r");
    if (fp != NULL) {
        if (::vfs.fgets(crcBuf, purefs::buffer::crc_char_size, fp) == NULL) {
            LOG_ERROR("verifyCRC ff_fgets on %s returned NULL", crcFilePath.c_str());
            return (false);
        }

        ::vfs.fclose(fp);
        const unsigned long crc32Read = strtoull(crcBuf, nullptr, purefs::buffer::crc_radix);
