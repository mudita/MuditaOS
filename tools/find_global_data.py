...
class ArchType(Enum):
    """Define supported architecture typs"""
...
class Finder():
    ...
    This class holds all neccecery variables and is responsible for holding and
    presenting results.
    ...
    def findStatic(self):
        ...
        for line in lines:
            ...
            if match:
                ...
                self.sections.put(("0x%x" % startAddress, "0x%x" % endAddres))
...
class StaticChecker(Thread):
    """Checks if found sections contain static sybols.

    This calass runs in separate processes objdump, deasembles section and search for hpp file.
    if found such place is loged.

    Attributes
    ----------

    finder : Finder
        access communication queues, and all other needed sttings
    hppRegexp : re
    ...
    def run(self):
        while True:
            ...
            if not self.finder.sections.empty():
                startAddres, stopAddress = self.finder.sections.get()
                try:
                    out = subprocess.run([self.finder.objdump,
                                          "--start-address="+startAddres,
