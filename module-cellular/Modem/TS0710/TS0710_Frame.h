    ...
    struct frame_t {
        ...
        std::vector<uint8_t> serialize() {
            ...
            if (Control == static_cast<uint8_t>(TypeOfFrame_e::UIH)) {
                len = 3;  //par. 5.3.6 of GSM0710 document states that for UIH frames, the FCS shall be calculated over only the address, control and length fields TODO: include 2-byte address
                // if (Length > 127)
                //     len += 1;
            }
...
public:
    ...
    frame_t getFrame() { return pv_frame; }
